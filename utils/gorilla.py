...
__all__ = [
    ...
    "get_decorator_data",
    "print_all_patched_diff",
    "get_all_patched_diff_str",
]
...
class Patch(object):
    ...
    def __repr__(self):
        return "{}(destination={!r}, name={!r}, obj={!r}, settings={!r})".format(
            type(self).__name__, self.destination, self.name, self.obj, self.settings
        )
...
def apply(patch, id="default"):
    ...
    try:
        ...
        if not settings.allow_hit:
            raise RuntimeError(
                ...
                "name so it can still be accessed.".format(
    ...
    use_replace_references = (
        hasattr(settings, "use_replace_references") and settings.use_replace_references
    )

    def normalize_code_block(lines: str) -> str:
        import black
        import textwrap

        code = textwrap.dedent(lines)

        try:
            # 使用 black 标准化格式
            mode = black.Mode()
            normalized = black.format_str(code, mode=mode)
        except Exception:
            raise ValueError("black.format_str error")

        # 去掉所有空白字符用于语义比较
        return normalized

    if callable(target):
        module_name = f"{patch.destination.__name__}"
        if not module_name.startswith("torch."):
            module_name = f"{patch.destination.__module__}.{module_name}"
        target_name = f"{module_name}.{patch.name}"
        target_codes = inspect.getsource(target)
        source_codes = inspect.getsource(patch.obj)
        target_codes_norm = normalize_code_block(target_codes)
        source_codes_norm = normalize_code_block(source_codes)
        diff = difflib.unified_diff(
            target_codes_norm.splitlines(keepends=True),
            source_codes_norm.splitlines(keepends=True),
            fromfile=target_name,
            tofile=f"{patch.name}",
        )
        _ALL_UNIFIED_DIFF.append("".join(diff))

        if (
            hasattr(get_original_function(target), "__code__")
            and not use_replace_references
        ):
            get_original_function(target).__code__ = get_original_function(
                patch.obj
            ).__code__
        else:
            replace_references(target, patch.obj)
    else:
        setattr(patch.destination, patch.name, patch.obj)
...
def revert(patch):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "'store_hit' was not set to True when applying the patch.".format(
...
# Call torch.compile to trigger all the monkey-patch.
def _trigger_all_patch():
    import torch
...
def print_all_patched_diff():
    _trigger_all_patch()
    for diff in _ALL_UNIFIED_DIFF:
        print(diff)


def get_all_patched_diff_str():
    _trigger_all_patch()
    return "\n".join(_ALL_UNIFIED_DIFF)


def check_patch_list(patches: List[str]) -> Dict[str, str]:
    """
    Check a list of function-level patches.
    Returns a dictionary mapping function_name -> result string.
    """
    results = {}
    REPLACE_PAIRS = {"cuda": "mlu", "gpu": "mlu"}
    replace_pattern = re.compile(r"(\bcuda\b|\bgpu\b)")

    def is_replaced_line(original: str, modified: str) -> bool:
        # Check if modified line is just original with cuda/gpu replaced by mlu
        if not original or not modified:
            return False  # Skip empty lines
        expected_modified = replace_pattern.sub(
            lambda m: REPLACE_PAIRS[m.group(1)], original
        )
        return expected_modified == modified
    ...
    for patch in patches:
        ...
        in_modify = False
        in_add = False
        pending_removed: Set[str] = set()  # unmatched '-' lines waiting for validation
        previous_removed: List[str] = []
        current_added: Set[str] = set()  # '+' lines inside Modify blocks
        ...
        try:
            ...
            for raw in lines_iter:
                ...
                # skip decorator block (+@ / -@ both)
                if stripped.startswith(("-@", "+@")):
                    for next_line in lines_iter:
                        # decorator block ends at function definition
                        if (
                            next_line.strip()
                            .lower()
                            .startswith(("def ", "-def ", "+def "))
                        ):
                            break
                    else:
                ...
                # handle "Modify by CAMBRICON" block start
                if re.search(r"#\s*Modify by CAMBRICON", lowered, re.I):
                    if in_modify:
                        raise RuntimeError(f"Nested Modify block found")
                    in_modify = True
                    continue

                if re.search(r"#\s*end Modify by CAMBRICON", lowered, re.I):
                    if not in_modify:
                        raise RuntimeError(f"end Modify without start")

                    if pending_removed:
                        pending_removed -= {
                            x.lstrip("#").strip() for x in current_added
                        }

                    in_modify = False
                    continue

                # handle "Add by CAMBRICON" block start
                if re.search(r"#\s*Add by CAMBRICON", lowered, re.I):
                    if in_add:
                        raise RuntimeError(f"Nested Add block found")
                    in_add = True
                    continue

                if re.search(r"#\s*end Add by CAMBRICON", lowered, re.I):
                    if not in_add:
                        raise RuntimeError(f"end Add without start")
                    in_add = False
                    continue

                # handle '-' lines
                if stripped.startswith("-"):
                    removed_line = stripped[1:].strip()
                    if removed_line:  # Skip empty lines
                        previous_removed.append(removed_line)
                    continue

                if stripped.startswith("+"):
                    added_line = stripped[1:].strip()
                    if added_line and previous_removed:
                        # check if it’s a cuda/gpu -> mlu replacement
                        last_removed = previous_removed[-1]
                        if is_replaced_line(last_removed, added_line):
                            previous_removed.pop()
                            continue
                    # collect '+' lines inside Modify blocks
                    if in_modify and added_line:
                        current_added.add(added_line)
                    continue

            if in_modify:
                raise RuntimeError("Modify block not closed")
            if in_add:
                raise RuntimeError("Add block not closed")

            pending_removed.update(previous_removed)
            pending_removed = {
                x for x in pending_removed if not any(x in y for y in current_added)
            }
            if pending_removed:
                raise RuntimeError(
                    f"Unmatched removed lines with no adjacent Modify block or + line: {pending_removed}"
                )
            pending_removed.clear()
            previous_removed.clear()
            current_added.clear()
...
def check_result():
    _trigger_all_patch()
    result = check_patch_list(_ALL_UNIFIED_DIFF)
