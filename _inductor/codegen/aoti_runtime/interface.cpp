...
#include "framework/inductor/aoti_runtime/model_container.h"
...
AOTIRuntimeError AOTInductorModelContainerCreate(
    ...
    size_t num_models,
    bool is_cpu,
    const char* cnbin_dir) {
  return AOTInductorModelContainerCreateWithDevice(
      container_handle, num_models, is_cpu ? "cpu" : "mlu", cnbin_dir);
}
...
AOTIRuntimeError AOTInductorModelContainerCreateWithDevice(
  ...
  CONVERT_EXCEPTION_TO_ERROR_CODE({
    std::optional<std::string> cnbin_dir_opt;
    if (cnbin_dir != nullptr) {
      cnbin_dir_opt.emplace(cnbin_dir);
    }
    auto* container = new torch::aot_inductor::AOTInductorModelContainer(
        num_models, std::string(device_str), cnbin_dir_opt);
    *container_handle =
