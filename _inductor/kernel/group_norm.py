...
group_norm_nhwc_tail_template = TritonTemplate(
    ...
    grid=group_norm_nhwc_grid,
    source=r"""
{{def_kernel("IN", "MEAN", "VAR", "TAIL_OUT", "WEIGHT", "BIAS", "NEW_VAR")}}
    pid = tl.program_id(0)
    pid_hw_id = tl.program_id(1)
    N = {{size("IN", 0)}}
    C = {{size("IN", 1)}}
...
def should_use_template(input, weight, bias):
    # TODO(hekai): Workaround for PYTORCH-12791
    input_size = input.get_size()
    # Modify by cambricon: check is_dynamic BEFORE is_nhwc.
    # is_nhwc() calls V.graph.sizevars.size_hints() which rejects SymInt values
    # (from FakeTensor.stride()) when the input has dynamic shapes, causing:
    #   torch.fx.experimental.symbolic_shapes or pass sympy expressions instead
    # The template does not support dynamic shape anyway, so return False early.
    if is_dynamic(input):
        return False
    input_stride = V.current_node.all_input_nodes[0].meta["val"].stride()
    if is_nhwc(input_stride):
        return False
...
def tuned_native_group_norm(
    ...
    if is_nchw(input_stride):
        ...
        input_nodes = [input, mean, var, tail_out, weight, bias, new_var]
        choices.clear()
        for GROUP_SIZE, ITER_NUM, HW_N, XBLOCK, num_warps, num_stages in configs:
