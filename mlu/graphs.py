...
if TYPE_CHECKING:
    ...
    from torch_mlu.mlu import _POOL_HANDLE
...
def is_current_stream_capturing() -> bool:
    r"""Return True if MLU graph capture is underway on the current MLU stream, False otherwise.
...
def graph_pool_handle() -> _POOL_HANDLE:
    ...
    """
    return torch.mlu._POOL_HANDLE(_graph_pool_handle())


# Python shim helps Sphinx process docstrings more reliably.
class MLUGraph(torch_mlu._MLUC._MLUGraph):
    r"""Wrapper around a MLU graph.

    Arguments:
        keep_graph (bool, optional): If ``keep_graph=False``, the
            cnrtTaskTopoEntity_t will be instantiated on MLU at the end of
            ``capture_end`` and the underlying cnrtTaskTopo_t will be
            destroyed. Users who want to query or otherwise modify the
            underlying cnrtTaskTopo_t before instantiation can set
            ``keep_graph=True`` and access it via ``raw_mlu_graph`` after
            ``capture_end``. Note that the cnrtTaskTopoEntity_t will not be
            ...
            is allowed to modify the raw cnrtTaskTopo_t after first calling
    ...
    def capture_begin(
        self, pool: _POOL_HANDLE | None = None, capture_error_mode: str = "relaxed"
    ) -> None:
        r"""Begin capturing MLU work on the current stream.
    ...
    def capture_end(self) -> None:
        r"""End MLU graph capture on the current stream.
    ...
    def instantiate(self) -> None:
        r"""Instantiate the MLU graph. Will be called by
        ``capture_end`` if ``keep_graph=False``, or by ``replay`` if
        ``keep_graph=True`` and ``instantiate`` has not already been
        explicitly called. Does not destroy the cnrtTaskTopo_t returned
        by ``raw_mlu_graph``.
        """
        super().instantiate()

    def replay(self) -> None:
        r"""Replay the MLU work captured by this graph."""
        super().replay()
    ...
    def enable_debug_mode(self) -> None:
        r"""Enable debugging mode for MLUGraph.debug_dump."""
    ...
    def raw_mlu_graph(self) -> int:
        r"""Returns the underlying cnrtTaskTopo_t. ``keep_graph`` must be True."""
        return super().raw_mlu_graph()

    def raw_mlu_graph_exec(self) -> int:
        r"""Returns the underlying cnrtTaskTopoEntity_t. ``instantiate`` must have been called if ``keep_graph`` is True, or ``capture_end`` must have been called if ``keep_graph`` is False. If you call ``instantiate()`` after ``raw_mlu_graph_exec()``, the previously returned cnrtTaskTopoEntity_t will be destroyed. It is your responsibility not to use this object after destruction."""  # noqa: B950
        return super().raw_mlu_graph_exec()


class graph:
    r"""Context-manager that captures MLU work into a :class:`torch.mlu.MLUGraph` object for later replay.

    See :ref:`MLU Graphs <mlu-graph-semantics>` for a general introduction,
    ...
    default_capture_stream: torch.mlu.Stream | None = None

    def __init__(
        self,
        mlu_graph: MLUGraph,
        pool: _POOL_HANDLE | None = None,
        stream: torch.mlu.Stream | None = None,
    ...
    def __enter__(self) -> None:
        ...
        if torch.compiler.config.force_cudagraph_gc:
            ...
            # multiple mlugraph captures in a row. In theory it will only help
            # when a dead python cycle is holding onto MLU memory.
            gc.collect()

        torch.mlu.empty_cache()

        torch_mlu._MLUC._host_emptyCache()
...
def make_graphed_callables(
    ...
    """
    if torch.is_autocast_enabled("mlu") and torch.is_autocast_cache_enabled():
        raise RuntimeError(
