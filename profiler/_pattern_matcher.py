...
class ExtraMLUCopyPattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.description = "Filled a CPU tensor and immediately moved it to MLU. Please initialize it on MLU."
        ...
        self.GPU_MIGRATION_PATTERN = re.compile(
            r"^torch_mlu/utils/gpu_migration/migration\.py\(\d+\): wrapper_fn$"
        )
        # for .mlu() method, that will call wrap_tensor_to additionally.
        self.WRAP_TENSOR_TO_PATTERN = re.compile(
            r"^torch/utils/backend_registration\.py\(\d+\): wrap_tensor_to$"
        )
    ...
    def match(self, event):
        ...
        if event is None:
            return False
        # When using .mlu() method or gpu_migration, we both will add one more stack frame
        while self.GPU_MIGRATION_PATTERN.search(
            event.parent.name
        ) or self.WRAP_TENSOR_TO_PATTERN.search(event.parent.name):
            event = event.parent
...
class FP32MatMulPattern(Pattern):
    ...
    @property
    def skip(self):
        return super().skip or not self.prof.record_shapes
