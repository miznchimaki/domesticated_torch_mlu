...
class DurationEvent(BaseEvent):
    def __init__(self, type, data):
        ...
        if extern_id is None:
            extern_id = self.args.get("External id", 0)
...
class KernelEvent(DurationEvent):
    def __init__(self, type, data):
        ...
        self.active_corenum = self.args.get("active_corenum", 0)
...
class OperatorEvent(DurationEvent):
    def __init__(self, type, data):
        ...
        self.input_strides = self.args.get("Input Strides", [])

        # Extract record_function_id for linking with ET nodes
        self.record_function_id = self.args.get("Record function id", 0)

        # Triton args
        self.is_triton_op = self.args.get("kernel_backend", "") == "triton"
        self.triton_args = TritonArgs(
            kernel_file=self.args.get("kernel_file", None),
            kernel_kwargs=self.args.get("kernel_kwargs", None),
            num_stages=self.args.get("num_stages", None),
            kernel_flop=self.args.get("kernel_flop", None),
            num_inputs=self.args.get("num_inputs", 0),
        )
...
class PythonFunctionEvent(DurationEvent):
    ...
    def contains(self, ts):
        return self.ts <= ts <= self.ts + self.duration
...
def create_trace_event(event) -> Optional[BaseEvent]:
    ...
    if event_type == EventTypes.USER_ANNOTATION:
        ...
        return OperatorEvent(event_type, event)
    elif event_type in [EventTypes.KERNEL, EventTypes.MEMCPY, EventTypes.MEMSET]:
