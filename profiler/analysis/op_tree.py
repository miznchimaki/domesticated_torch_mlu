    ...
    def _set_annotation_parent(self, tail_node, cncl_anno_list):
        for cncl_anno in cncl_anno_list:
            if (
                ...
                if tail_node.name == "record_param_comms":
                    cncl_anno.set_parent(tail_node.parent_node)
                else:
    ...
    def _build_tree(
        ...
        for tid, op_list in tid2list.items():
            ...
            root_node = self._build_tree_internal(
                op_list, zero_rt_list, tid, [], cncl_anno_list, data_context
            )
    ...
    def _build_tree_internal(
        ...
        def build_tree_relationship(
            ...
            if staled_device_nodes:
                ...
                dummpy_rt[0].fill_stats(None, data_context)
                    ...
                    if node.start_time < tail_node.end_time:
                        if node.end_time <= tail_node.end_time:
                            ...
                            if node.name == "record_param_comms":
                                # set cpu-op "c10d::*" to its parent node, for better show in kernel_details.csv
        ...
        root_node = build_tree_relationship(
            host_node_list, zero_rt_list, staled_device_nodes, data_context
        )
        remove_dup_nodes(root_node)
        root_node.fill_stats(data_context)
