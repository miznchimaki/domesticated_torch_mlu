...
#include "framework/inductor/aoti_runtime/device_utils.h"
...
RAIIDataPtr RAII_mluMalloc(size_t num_bytes) {
#ifdef AOT_INDUCTOR_USE_CACHING_ALLOCATOR
  // Use caching allocator for allocating MLU memory
  void* data_ptr = nullptr;
  AOTI_TORCH_ERROR_CODE_CHECK(
      aoti_torch_mlu_caching_allocator_raw_alloc(num_bytes, &data_ptr));
  auto deleter = [](void* ptr) {
    AOTI_TORCH_ERROR_CODE_CHECK(
        aoti_torch_mlu_caching_allocator_raw_delete(ptr));
  };
  return RAIIDataPtr(data_ptr, deleter);
#else
  void* data_ptr;
  AOTI_RUNTIME_MLU_CHECK(cnrtMalloc((void**)&data_ptr, num_bytes));
  auto deleter = [](void* ptr) { AOTI_RUNTIME_MLU_CHECK(cnrtFree(ptr)); };
  return RAIIDataPtr(data_ptr, deleter);
#endif
}
...
class AOTInductorModelBase {
 public:
  AOTInductorModelBase(
    ...
    if (device_idx_ == -1) {
      AOTI_RUNTIME_MLU_CHECK(cnrtGetDevice(&device_idx_));
    } else {
      // If device_idx_ is passed in, we need to set the current device to it
      AOTI_RUNTIME_MLU_CHECK(cnrtSetDevice(device_idx_));
    }
  }
  ...
  ~AOTInductorModelBase() {
    if (run_finished_) {
      auto code = cnrtNotifierDestroy(*run_finished_);
      if (code != cnrtSuccess) {
        std::cerr << "Failed to destroy CNRT notifier in AOTInductor model: "
                  << cnrtGetErrorStr(code) << std::endl;
      }
    }
  }
  ...
  void run(
    ...
    if (!run_finished_) {
      cnrtNotifier_t run_finished;
      AOTI_RUNTIME_MLU_CHECK(cnrtNotifierCreate(&run_finished));
    ...
    AOTI_RUNTIME_MLU_CHECK(cnrtPlaceNotifier(*run_finished_, stream));
  }
  ...
  void load_constants(bool force = false) {
    ...
    constant_blob_ = RAII_mluMalloc(blob_size);
  ...
  uint8_t* constant_ptr(
    ...
    if (!skip_copy) {
      AOTI_RUNTIME_MLU_CHECK(cnrtMemcpy(
          internal_ptr,
  ...
  bool is_finished() {
    if (!run_finished_) {
      throw std::runtime_error{"Model MLU Notifier was not initialized"};
    }

    auto event_status = cnrtQueryNotifier(*run_finished_);
    if (event_status == cnrtSuccess) {
      return true;
    } else if (event_status == cnrtErrorNotReady) {
      return false;
    }
  ...
  void wait_for_completion() {
    ...
    AOTI_RUNTIME_MLU_CHECK(cnrtWaitNotifier(*run_finished_));
  }
 ...
 protected:
  ...
  // A directory with mLU binary files, e.g. compiled kernels, etc.
  ...
  std::optional<cnrtNotifier_t> run_finished_;

  // Generated model uses this device index to create MLU guards.
